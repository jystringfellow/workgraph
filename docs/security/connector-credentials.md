...
| `~/.workgraph/llm.json` | LLM profile metadata such as provider, base URL, model, signed-in client config-dir path, AWS profile, Bedrock model ARN, and API key environment variable names | No raw API keys or client login tokens should be stored here. OpenAI-compatible hosted credentials should use environment variable references such as `api_key_env`; Bedrock and signed-in clients use their local credential resolution. |
| `~/.workgraph/bridge/workers.json` | Non-secret worker model and signed-in client config-dir bindings | Does not store provider or client login tokens. Use plugin install model/config-dir clear flags to remove bindings. |
