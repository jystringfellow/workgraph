...
description: Configure and execute local workgraph capture through provider connectors already approved in Claude Code.
---

# workgraph bridge

Workgraph owns cadence, bounded requests, cursors, dedupe, and local storage.
Use Claude Code provider connectors only to execute requests exposed by the
local `workgraph` MCP server.
