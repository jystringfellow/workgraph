...
4. (Optional) Enter your app's App Store ID if the app is published in Apple's App Store. The Store ID
   ...
   4. Select **Copy Link**.
   5. Paste the link into a text editor. The App Store ID is the final part of the URL. Example: `https://apps.apple.com/app/google/id284815942`

5. (Optional) Enter your Team ID. See
...
6. (Optional) Enable App Check for your iOS app. When you enable App Check, Apple's
...
authorization request, and its transformed value, called "code_challenge", is sent to the
...
- An unsupported custom scheme was used for the redirect uri. If you see the error message **Custom URI scheme is not supported on Android or Chrome apps** , [learn more](https://developers.google.com/identity/protocols/oauth2/native-app#redirect-uri_custom-scheme) about custom URI scheme alternatives.
...
3. **Create the DPoP payload:**
   ...
   - For **authorization code exchanges** , the `jti` must be the Base64Url-encoded SHA256 hash of the authorization code: `"jti":"BASE64URL(SHA256(AUTHORIZATION_CODE))"`.
...
> [!NOTE]
> **Note:** If your application already has a refresh token for the user and you want to obtain a new DPoP-bound refresh token, the user must revoke the existing grant or you must use the `prompt=consent` parameter in the initial authorization request to ensure a new refresh token is issued.
...
> [!WARNING]
> **Important:** The loopback IP address redirect option is DEPRECATED for **Android** , **Chrome app** and **iOS** OAuth client types. Review the [loopback IP
...
> [!NOTE]
> **Note:** See the [`redirect_uri`](https://developers.google.com/identity/protocols/oauth2/native-app#request-parameter-redirect_uri) parameter definition for more information about the loopback IP address. It is also possible to use `localhost` in place of the loopback IP, but this configuration may cause issues with client firewalls. Most, but not all, firewalls allow loopback communication.
